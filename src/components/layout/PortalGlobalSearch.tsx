    ...
    {open && <div className="fixed inset-0 z-50 flex items-start justify-center bg-black/50 p-2 pt-3 sm:p-4 sm:pt-[10vh]" onClick={(event) => { if (event.target === event.currentTarget) setOpen(false); }}>
      <div className="flex max-h-[calc(100vh-1.5rem)] w-full max-w-2xl flex-col overflow-hidden rounded-xl border border-border bg-white shadow-2xl" role="dialog" aria-modal="true" aria-label="Global search">
        ...
        <div className="min-h-[12rem] overflow-y-auto py-2">
          {term.trim().length < 2 ? <div className="px-4 py-10 text-center"><FileSearch className="mx-auto mb-3 h-8 w-8 text-muted-foreground/70" /><p className="text-sm text-muted-foreground">Search across whistleblowing cases.</p></div> : query.isFetching ? <div className="flex items-center justify-center gap-2 px-4 py-10 text-sm text-muted-foreground"><Loader2 className="h-4 w-4 animate-spin" />Searching cases…</div> : query.isError ? <p className="px-4 py-10 text-center text-sm text-destructive">Search is unavailable right now.</p> : cases.length === 0 ? <p className="px-4 py-10 text-center text-sm text-muted-foreground">No cases match “{term.trim()}”.</p> : <div>{cases.map((item) => <button key={item.id} type="button" onClick={() => go(item.id)} className="flex w-full items-start gap-3 px-4 py-3 text-start hover:bg-muted/60"><span className="rounded-md bg-brand-accent/10 p-2 text-brand-accent"><FileSearch className="h-4 w-4" /></span><span className="min-w-0 flex-1"><strong className="block truncate text-sm text-foreground">{item.caseReferenceNumber}</strong><span className="block truncate text-xs text-muted-foreground">{item.category.replaceAll('_', ' ')} · {item.status.replaceAll('_', ' ')}</span></span></button>)}</div>}
        </div>
        <div className="flex items-center gap-4 border-t border-border bg-muted/50 px-4 py-2 text-[11px] text-muted-foreground/70"><span className="flex items-center gap-1"><ArrowUp className="h-3 w-3" /><ArrowDown className="h-3 w-3" />Navigate</span><span className="flex items-center gap-1"><CornerDownLeft className="h-3 w-3" />Open</span><span>Esc Close</span></div>
      </div>
    </div>}
  </>;
}
