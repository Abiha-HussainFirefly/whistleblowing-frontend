...
function Stat({ label, value, hint, icon: Icon }: { label: string; value: string | number; hint: string; icon: typeof Users }): ReactElement {
  return <Card className="p-4"><div className="flex items-start justify-between gap-3"><div><p className="text-[10px] font-medium uppercase tracking-wide text-slate-400">{label}</p><p className="mt-2 text-2xl font-semibold text-slate-900">{value}</p><p className="mt-1 text-xs text-slate-400">{hint}</p></div><span className="flex h-8 w-8 items-center justify-center rounded-md bg-[#e5f4f5] text-brand-accent"><Icon className="h-4 w-4" /></span></div></Card>;
}

function Pie({ percent, label, value }: { percent: number; label: string; value: string }): ReactElement {
  const safe = Math.min(100, Math.max(0, percent));
  return <div className="flex items-center gap-5"><div className="relative h-24 w-24 shrink-0 rounded-full" style={{ background: `conic-gradient(#078995 ${safe}%, #e7edf3 0)` }}><div className="absolute inset-3 rounded-full bg-white" /></div><div className="min-w-0"><div className="flex items-center gap-2 text-xs text-slate-500"><span className="h-2 w-2 rounded-full bg-brand-accent" />{label}</div><p className="mt-1 text-sm font-semibold text-slate-700">{value}</p></div></div>;
}
...
function Donut({ admins, users }: { admins: number; users: number }): ReactElement {
  ...
  return <div className="flex items-center gap-5"><div className="relative h-24 w-24 shrink-0 rounded-full" style={{ background: `conic-gradient(#078995 ${adminPercent}%, #3f7fec 0)` }}><div className="absolute inset-3 flex flex-col items-center justify-center rounded-full bg-white"><strong className="text-lg text-slate-800">{total}</strong><span className="text-[9px] text-slate-400">Total</span></div></div><div className="space-y-2 text-xs text-slate-500"><div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-brand-accent" />Admins {admins} ({adminPercent}%)</div><div className="flex items-center gap-2"><span className="h-2 w-2 rounded-full bg-[#3f7fec]" />Users {users} ({total === 0 ? 0 : 100 - adminPercent}%)</div></div></div>;
}
...
export function OrgAdminDashboardPage(): ReactElement {
  ...
  return <div className="space-y-5">
    <div><PageTitle className="text-xl text-brand-primary">Welcome, {displayName}.</PageTitle><p className="mt-1 text-sm text-slate-500">Organization management console for {organization?.name ?? 'your organization'}. Manage members, oversight, roles, regions, and settings from here.</p>{activeRegion !== null && <span className="mt-2 inline-flex rounded-full bg-white px-2.5 py-1 text-[10px] text-slate-500">Active region: {activeRegion}</span>}</div>
    {loading && <Loader label="Loading organization overview..." />}
    {error && <div className="rounded-lg border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700">{getApiErrorMessage(error)}</div>}
    {!loading && !error && <>
